...
export function addMessageHandler(m, sock) {
  ...
  m.reply = async (text, quoted = true, useContext = true, newsletterName = 'Sonata Bot') => {
  ...
  // Method untuk interactive button
  m.sendInteractiveButton = async (text, buttons = [], opts = {}) => {
    const defaultOpts = {
      ...
      sections: [], // Untuk list sections
      buttonText: 'Pilih Menu', // Text untuk list button
      externalAdReply: {
  ...
  m.sendListMessage = async (text, sections = [], opts = {}) => {
    const defaultOpts = {
      ...
      externalAdReply: {
        ...
        body: "Simple WhatsApp Bot",
        thumbnailUrl: "https://telegra.ph/file/8360caca1efd0f697d122.jpg",
        sourceUrl: "https://github.com/Kanata-chan",
        mediaType: 1,
        renderLargerThumbnail: true
      }
    }
...
function getMessageType(message) {
  ...
  // Cek view once message
  if (message.viewOnceMessageV2?.message) {
    const vo = message.viewOnceMessageV2.message;
    const voType = Object.keys(vo)[0];
    return voType.replace('Message', '');
  }
