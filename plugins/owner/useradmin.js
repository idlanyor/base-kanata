...
export default {
  name: 'useradmin',
  alias: ['adminuser', 'usermod'],
  category: 'owner',
  desc: 'Admin user management commands',
  use: '<ban/unban/warn/search/info> [@user] [reason]',
  async exec({ sock, m, args, prefix }) {
    try {
      ...
      if (!args.length) {
        return await m.reply(`❌ *Invalid Usage*\n\nUsage: ${prefix}useradmin <command> [@user] [reason]\n\nCommands:\n• ban - Ban a user\n• unban - Unban a user\n• warn - Warn a user\n• search - Search users\n• info - Get user info`)
      }
      ...
      switch (command) {
        case 'ban':
          if (!targetJid) {
            return await m.reply('❌ *Invalid Usage*\nPlease mention a user to ban.\n\nUsage: ' + prefix + 'useradmin ban @user [reason]')
          }
...
The user has been banned from using the bot.`)
        ...
        case 'unban':
          if (!targetJid) {
            return await m.reply('❌ *Invalid Usage*\nPlease mention a user to unban.\n\nUsage: ' + prefix + 'useradmin unban @user')
          }
...
The user can now use the bot again.`)
        ...
        case 'warn':
          if (!targetJid) {
            return await m.reply('❌ *Invalid Usage*\nPlease mention a user to warn.\n\nUsage: ' + prefix + 'useradmin warn @user [reason]')
          }
...
${warnedUser.warnings >= 3 ? '🚫 *User has been auto-banned due to 3 warnings!*' : '⚠️ *User will be auto-banned after 3 warnings.*'}`)
        ...
        case 'search':
          if (!args[1]) {
            return await m.reply('❌ *Invalid Usage*\nPlease provide a search query.\n\nUsage: ' + prefix + 'useradmin search <query>')
          }
